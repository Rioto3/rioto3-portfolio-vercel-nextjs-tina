...
なお、本研究には明確な限界があることを認識しておく必要があります。今回の分析は宮城県という一地域に限定されたものであり、全国的な傾向を代表するものではありません。また、gBizINFO APIのデータ更新の遅延により、分析対象期間にはばらつきがあり、最新の動向を完全に反映していない可能性があります。これらの制約を踏まえた上で、今後は複数地域のデータ収集や時系列分析を通じて、より包括的な知見を得ることが課題となります。
